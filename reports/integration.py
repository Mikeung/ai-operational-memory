...
def generate_integration_readiness_report(
    ...
    if active_projects:
        lines.append(f"| Project ID | Retention Profile | Ingestion Enabled |")
        lines.append(f"|---|---|---|")
...
def generate_ingestion_compatibility_report(
    ...
    if flagged == 0:
        ...
        for i, (sample, issues) in enumerate(zip(event_samples, issues_by_sample)):
            if not issues:
                continue
...
def generate_sdk_usage_guidance(
    ...
    lines = [
        ...
        "`ai-operational-memory` repository. The SDK is available at `sdk/python/`.",
        "",
        "## Basic Usage",
        "",
        "```python",
        ...
        "",
        f"client = OperationalMemoryClient(",
        f"    base_url='{base_url}',",
