...
Day-to-day operational reference for AI Operational Memory.
...
**Systemd health**: `systemctl status ai-operational-memory`

**Log streaming**: `journalctl -u ai-operational-memory -f`

**Cron-based health check**:
```cron
# Alert if server goes down
*/5 * * * * /opt/ai-operational-memory/scripts/healthcheck.sh || echo "ALERT: ops server down" | mail -s "ops-alert" ops@example.com
```

**Weekly backup cron**:
```cron
0 3 * * 0 /opt/ai-operational-memory/scripts/backup.sh
```
