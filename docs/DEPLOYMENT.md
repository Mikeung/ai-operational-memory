...
This guide covers deploying AI Operational Memory on a single VPS.
...
```bash
git clone <repo-url> ai-operational-memory
cd ai-operational-memory
...
```ini
# /etc/systemd/system/ai-operational-memory.service
[Unit]
Description=AI Operational Memory
After=network.target

[Service]
Type=simple
User=deploy
WorkingDirectory=/opt/ai-operational-memory
ExecStart=/opt/ai-operational-memory/.venv/bin/uvicorn backend.main:app --host 127.0.0.1 --port 8000
Restart=on-failure
RestartSec=10
EnvironmentFile=/opt/ai-operational-memory/.env
...
```bash
sudo systemctl enable ai-operational-memory
sudo systemctl start ai-operational-memory
```
...
./scripts/update.sh
# then restart
sudo systemctl restart ai-operational-memory
```
...
```bash
sudo systemctl stop ai-operational-memory
